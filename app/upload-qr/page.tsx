...
export default function UploadQRPage() {
  ...
  function needsPaymentMessage() {
    return (
      <>
        Necesitas completar el pago para generar el QR.{" "}
        <a href="/pricing?need=valid" style={{ textDecoration: "underline" }}>
          Ir a pagar
        </a>
        .
      </>
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (initRes.status === 402) {
        setMsg({ text: "", type: "" });
        setEmailMsg({ text: "", type: "" });
        return setMsg({ text: (needsPaymentMessage() as any), type: "error" } as any);
      }
      const initData = await initRes.json().catch(() => null);
      if (!initRes.ok || !initData?.ok) {
        return setMsg({
          text: initData?.error || initData?.detail || `No se pudo iniciar la subida (INIT ${initRes.status}).`,
          type: "error",
        });
      }
      ...
      if (putRes.status === 402) return setMsg({ text: (needsPaymentMessage() as any), type: "error" } as any);
      ...
      if (!ETag) return setMsg({ text: "No se recibió ETag del PUT (requerido para completar).", type: "error" });
      ...
      if (completeRes.status === 402) return setMsg({ text: (needsPaymentMessage() as any), type: "error" } as any);
      const completeData = await completeRes.json().catch(() => null);
      if (!completeRes.ok || !completeData?.ok) {
        return setMsg({
          text: completeData?.error || completeData?.detail || `Fallo al completar la subida (COMPLETE ${completeRes.status}).`,
          type: "error",
        });
      }
      ...
      if (signRes.status === 402) return setMsg({ text: (needsPaymentMessage() as any), type: "error" } as any);
      ...
      <div style={{ marginTop: 16, padding: 12, border: "1px solid #e5e7eb", borderRadius: 8, maxWidth: 480 }}>
        ...
        {!key && (
          <p style={{ marginTop: 8, color: "#6b7280" }}>
            Sube un archivo (y paga) para habilitar el envío por email.
          </p>
        )}
